...
const uint32_t may_war_set_fft[NUM_TEB_FFT][2];
...
TEB(task_init);
TEB(task_sample);
TEB(task_fft_prologue);
TEB(task_fft_stage_1);
TEB(task_fft_stage_2);
TEB(task_fft_stage_3);
TEB(task_fft_epilogue);
TEB(task_done);
...
TEB(task_sample)//1
{
	...
	NEXT(task_fft_prologue);
}


TEB(task_fft_prologue)//2
{
    uint16_t i, length;
    ...
    NEXT(task_fft_stage_1);
}


TEB(task_fft_stage_1)//3
{
    uint16_t length;
    ...
    NEXT(task_fft_stage_2);
}


TEB(task_fft_stage_2)//4
{
    uint16_t length;
    bool scale;                         // scale flag
    bool overflow;                      // overflow flag
    ...
    NEXT(task_fft_stage_3);
}


TEB(task_fft_stage_3)//5
{
    uint16_t length;
    bool scale;                         // scale flag
    bool overflow;                      // overflow flag
    ...
    NEXT(task_fft_epilogue);
}


TEB(task_fft_epilogue)//6
{
    uint16_t i;
...
void _benchmark_fft_init()
{
    ...
    //msp_gpio_unlock();
    TASK(TASK_PRI, task_init);

    TEB_INIT(TASK_PRI, task_init, 1, may_war_set_fft[0][0], may_war_set_fft[0][1]);       //0
    TEB_INIT(TASK_PRI, task_sample, 21, may_war_set_fft[1][0], may_war_set_fft[1][1]); //1
    TEB_INIT(TASK_PRI, task_fft_prologue, 33, may_war_set_fft[2][0], may_war_set_fft[2][0]); //2
    TEB_INIT(TASK_PRI, task_fft_stage_1, 33, may_war_set_fft[3][0], may_war_set_fft[3][0]);    //3
    TEB_INIT(TASK_PRI, task_fft_stage_2, 31, may_war_set_fft[4][0], may_war_set_fft[4][0]);  //4
    TEB_INIT(TASK_PRI, task_fft_stage_3, 64, may_war_set_fft[5][0], may_war_set_fft[5][0]); //5
    TEB_INIT(TASK_PRI, task_fft_epilogue, 36, may_war_set_fft[6][0], may_war_set_fft[6][0]);  //6
    TEB_INIT(TASK_PRI, task_done, 1, may_war_set_fft[7][0], may_war_set_fft[7][0]);      //7
#if ENABLE_PRF
    PRF_INIT();
    PRF_POWER();
#endif
